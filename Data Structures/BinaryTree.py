# -*- coding: utf-8 -*-
"""
Created on Sat Nov 13 21:35:31 2021
...
def is_bst(node):
    ...
    max_key = max(remove_none([min_l, node.key, min_r]))
...
def tree_sizes(node):
    if node is None:
        return 0
    return 1 + tree_sizes(node.left) + tree_sizes(node.right)
